 ...
 */
int dir_is_empty(const wchar_t *dir) {
  ...
  do {
    if (wcscmp(file_desc.cFileName, L".") != 0 &&
        wcscmp(file_desc.cFileName, L"..") != 0) {
...
int wmain(const int argc, wchar_t *argv[]) {
  ...
  do {
    ...
    if (!wcscmp(file_desc.cFileName, L".") ||
        !wcscmp(file_desc.cFileName, L"..")) {
      continue; /* skip . and .. */
    };

    wchar_t sub[MAX_PATH];
    if (_snwprintf_s(sub, MAX_PATH, _TRUNCATE, L"%s\\%s", argv[1],
                     file_desc.cFileName) < 0) {
      continue;
    };
